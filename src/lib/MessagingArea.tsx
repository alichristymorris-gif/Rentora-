...
// Supabase initialized globally in ./supabase
...
export function MessagingArea({ user, initialChatId }: MessagingAreaProps) {
  ...
  useEffect(() => {
    ...
    channel = supabase.channel(`chats-sessions-${user.uid}`)
  ...
  useEffect(() => {
    ...
    channel = supabase.channel(`chat-messages-${activeChatId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'messages', filter: `chatId=eq.${activeChatId}` }, () => {
        fetchMessages();
      })
      .subscribe();
  ...
  useEffect(() => {
    ...
    channel = supabase.channel(`partner-user-${partnerId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'users', filter: `uid=eq.${partnerId}` }, () => {
        fetchPartner();
      })
      .subscribe();
  ...
  const handleEnteringText = async (text: string) => {
    setInputText(text);
    if (!activeChatId) return;

    const { data: chat } = await supabase.from('chats').select('typing').eq('id', activeChatId).maybeSingle();
    const typing = chat?.typing || {};
    typing[user.uid] = text.length > 0;

    await supabase.from('chats').update({ typing }).eq('id', activeChatId);
  };

  const clearLocalTyping = async () => {
    if (!activeChatId) return;
  ...
  const handleTogglePinChat = async (e: React.MouseEvent, session: ChatSession) => {
    ...
    const pinnedBy = Array.isArray(session.pinnedBy)
      ? (isPinned ? session.pinnedBy.filter(id => id !== user.uid) : [...session.pinnedBy, user.uid])
      : [user.uid];
    
    await supabase.from('chats').update({ pinnedBy }).eq('id', session.id);
  };
  ...
  return (
    <div className="flex flex-1 h-[680px] bg-white dark:bg-dark-surface rounded-[40px] border border-bento-border dark:border-dark-border overflow-hidden transition-colors shadow-2xl relative">
      <input 
        type="file" 
        className="hidden" 
        ref={fileInputRef} 
          ...
          {filteredSessions.map((session) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full p-4 rounded-2xl flex items-center gap-3 transition-all group border text-left",
                  isActive 
          ...
          <>
                    ...
                    {showOptionsDropdown && (
                      ...
                      >
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-3.5 py-2.5 text-xs font-bold text-rose-500 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/20 transition-all text-left"
                        >
                          <UserX size={14} />
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-3.5 py-2.5 text-xs font-bold text-slate-600 dark:text-dark-muted rounded-lg hover:bg-slate-50 dark:hover:bg-slate-850 transition-all text-left"
                        >
                          <Flag size={14} />
                          Report User
                        </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 no-scrollbar">
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id} className={cn("flex items-end gap-2.5 group", isMine ? "flex-row-reverse animate-slide-in-right" : "flex-row animate-slide-in-left")}>
                          ...
                          editingMessageId === msg.id ? (
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-1.5 justify-end">
                                <button onClick={() => setEditingMessageId(null)} className="px-3 py-1 bg-slate-200 dark:bg-slate-707 text-[10px] text-slate-705 dark:text-white font-bold rounded">Cancel</button>
              ...
              <div className="space-y-4 text-left">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    ...
                    className="w-full p-4 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-dark-border rounded-xl text-xs font-semibold text-slate-800 dark:text-white outline-none resize-none"
                  />
                </div>
