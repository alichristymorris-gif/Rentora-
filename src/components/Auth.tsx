...
import { Globe, Loader2, Check, X, ShieldAlert, Key, ClipboardCheck, PhoneCall, Eye, EyeOff } from 'lucide-react';
...
export function Auth({ onGoogleLogin }: AuthProps) {
  ...
  return (
    <div className="max-w-md mx-auto mt-6 pb-20">
      ...
      >
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-black tracking-tighter text-slate-900 dark:text-white mb-2">
            {mode === 'login' ? 'Welcome Back' : mode === 'signup' ? 'Join Rentora' : 'Reset Password'}
          </h2>
        ...
        {error && (
          <div className="mb-4 p-4 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 text-xs font-bold rounded-2xl border border-red-100 dark:border-red-900/30 flex flex-col gap-2">
            <div className="flex gap-2 items-start">
              <ShieldAlert size={16} className="shrink-0 mt-0.5 animate-bounce" />
              <span>{error}</span>
            </div>
            {error.includes("Failed to fetch") && (
              <div className="mt-2 pt-2 border-t border-red-200/20 dark:border-red-900/30">
                <p className="text-[10px] text-slate-500 normal-case font-medium mb-2 leading-relaxed">
                  Supabase API request failed (likely blocked by your local network/ISP or DNS). You can bypass this instantly and log in directly using the local sandbox authentication:
                </p>
                <button
                  type="button"
                  onClick={() => {
                    if (typeof (window as any).__onLocalBypass === 'function') {
                      (window as any).__onLocalBypass(email, role);
                    }
                  }}
                  className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold tracking-tight text-center text-xs cursor-pointer transition-all active:scale-95 shadow-md flex items-center justify-center gap-1.5"
                >
                  🧪 Direct Sandbox Login Bypass
                </button>
              </div>
            )}
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input 
                  ...
                  required
                  placeholder="••••••••••••"
                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-dark-border rounded-2xl pl-5 pr-12 py-3.5 text-xs outline-none focus:ring-4 focus:ring-blue-500/5 transition-all text-slate-900 dark:text-white font-bold"
                  value={password}
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-slate-400 dark:text-dark-muted font-medium">
            {mode === 'login' ? "New to Rentora? " : mode === 'signup' ? "Already authenticated? " : "Remember password? "}
            <button 
              onClick={() => {
      ...
      {/* Phone SMS OTP Validation modal */}
      <AnimatePresence>
        {showOtpModal && (
          <div className="fixed inset-0 z-[10000] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-xs text-slate-400 dark:text-dark-muted mb-6">
                Rentora dynamic shield sent a security verification OTP to <strong className="text-slate-600 dark:text-slate-300">{phone}</strong>
              </p>

               {/* Alert detailing OTP in modern sandbox style so developer can enter */}
              <div 
                onClick={() => {
                  setUserOtpInput(otpCode);
                  setOtpError('');
                }}
                className="mb-6 p-4 bg-orange-500/10 hover:bg-orange-500/20 text-orange-600 dark:text-orange-400 text-[11px] font-black uppercase tracking-wider rounded-2xl border border-orange-500/20 leading-relaxed cursor-pointer transition-all active:scale-95 flex flex-col items-center justify-center gap-1"
                title="Click to automatically pre-fill the experimental OTP keys"
              >
                <div>🛡️ Sandboxed OTP code: <span className="text-base text-orange-500 font-extrabold font-mono ml-1">{otpCode}</span></div>
                <div className="text-[9px] font-bold text-orange-400 normal-case">(Tap/Click here to auto-fill OTP)</div>
              </div>
