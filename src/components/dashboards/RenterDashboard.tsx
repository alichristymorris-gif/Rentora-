...
export function RenterDashboard({ 
  ...
  useEffect(() => {
    ...
    requestsChannel = supabase.channel(`renter-requests-${user.uid}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'rentalRequests', filter: `renterId=eq.${user.uid}` }, () => {
        fetchRequests();
      })
      .subscribe();
