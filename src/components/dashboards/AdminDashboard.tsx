...
export function AdminDashboard({ user, onLogout, listings: allListings, onSelectListing }: AdminDashboardProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'listings' | 'reports' | 'agents'>('overview');
  ...
  useEffect(() => {
    ...
    usersChannel = supabase.channel('admin-users')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'users' }, () => {
        fetchUsers();
      })
      .subscribe();

    reportsChannel = supabase.channel('admin-reports')
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-4">
            <button onClick={onLogout} className="text-red-600 text-sm font-black flex items-center gap-2 hover:bg-red-50 dark:hover:bg-red-900/20 px-6 py-3 rounded-2xl transition-all border border-transparent hover:border-red-100 cursor-pointer">
        ...
        <div className="flex items-center gap-2 mb-12 overflow-x-auto no-scrollbar pb-2">
          {[
            ...
            { id: 'agents', label: 'Agent Panel 🟡', icon: Bot },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
              className={cn(
                "flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-black transition-all whitespace-nowrap cursor-pointer",
                activeTab === tab.id 
                               ...
                               <td className="px-8 py-6">
                                  <button 
                                     ...
                                     className={cn(
                                        ...
                                        u.isBanned 
                                          ? "bg-emerald-50 dark:bg-emerald-950/25 text-emerald-600 hover:bg-emerald-100/30" 
                                          : "bg-red-50 dark:bg-red-950/25 text-red-600 hover:bg-red-100/30"
                                     )}
                                  >
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-sans text-xs">
                {allListings.map((l: any) => (
                   <div key={l.id} className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-dark-border overflow-hidden group transition-colors">
                      ...
                      <div className="p-6">
                         <h4 className="font-black text-slate-900 dark:text-white mb-1 truncate text-sm">{l.title}</h4>
                         <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-6">by {l.ownerName}</p>
                         <div className="flex gap-2">
                            <button 
                               onClick={() => onSelectListing(l)}
                               className="w-11 h-11 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-400 hover:text-slate-900 transition-all cursor-pointer"
                               title="View Listing"
                            >
                               <Eye size={18} />
                            </button>
                            <button 
                               ...
                               className="flex-1 bg-emerald-50 dark:bg-emerald-950/25 text-emerald-600 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100/30 cursor-pointer"
                            >
                               Approve
                            </button>
                            <button 
                               onClick={() => handleModeration(l.id, 'banned')}
                               className="flex-1 bg-red-50 dark:bg-red-950/25 text-red-600 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-100/30 cursor-pointer"
                            >
                               Ban
                            </button>
                         </div>
                               ...
                               {r.status === 'pending' ? (
                                  <>
                                     <button 
                                        ...
                                        className="px-6 py-3 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 cursor-pointer"
                                     >
                                        Resolve
                                     </button>
                                     <button 
                                        onClick={() => handleResolveReport(r.id, 'dismissed')}
                                        className="px-6 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 cursor-pointer"
                                     >
                                        Dismiss
                                     </button>
                                  </>
